import { useShoeContext } from '../contexts/shoes-context'

export default function CartItem({data}){
    const { deleteItemCart, decrementQuantity, incrementQuantity } = useShoeContext();

    return (
        <div key={data.id} className="cardItem">
                                <div className="cardItem_left">
                                    <div className="cardItem_image"  style={{ backgroundColor: data.color }}>
                                        <img
                                            alt=""
                                            src={data.image}
                                        />
                                    </div>
                                </div>
                                <div className="cardItem_right">
                                    <div className="cardItem_name">
                                        {data.name}
                                    </div>
                                    <div className="cardItem_price">{data.price}</div>
                                    <div className="cartItem_actions">
                                    <div className="cartItem_count">
                                        <div className="cartItem_button" onClick={() => decrementQuantity(data.id)}>-</div>
                                        <div className="cartItem_number">{data.quantity}</div>
                                        <div className="cartItem_button" onClick={() => incrementQuantity(data.id)}>+</div>
                                    </div>
                                    <div className="carItem_remove" onClick={() => deleteItemCart(data.id)}>
                                        <img
                                            alt=""
                                            src="data:image/png;base64,"
                                        />
                                    </div>
                                    
                                    </div>
                                </div>
                            </div>

    )
}